...
def encode_tf(labels, priors, match_thresh, ignore_thresh,
               variances=[0.1, 0.2]):
    """tensorflow encoding"""
    # labels = [facebox[4], landmark[68*2=136], param[62], valid[1]]
    
    assert ignore_thresh <= match_thresh
    priors = tf.cast(priors, tf.float32)
    bbox = labels[:, :4]
    landm = labels[:, 4:4+136]
    ...
    matches_param = tf.gather(param, best_truth_idx) # [num_priors, 62]
    ...
    return tf.concat([loc_t, landm_t, matches_param, 
                      landm_valid_t[..., tf.newaxis], conf_t[..., tf.newaxis]], axis=1)
...
def _encode_landm(matched, priors, variances):
    ...
    # dist b/t match center and prior's center
    matched = tf.reshape(matched, [tf.shape(matched)[0], 68, 2])
    priors = tf.broadcast_to(
        tf.expand_dims(priors, 1), [tf.shape(matched)[0], 68, 4])
